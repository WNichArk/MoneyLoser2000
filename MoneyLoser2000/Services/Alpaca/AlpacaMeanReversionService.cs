...
{
    internal sealed class AlpacaMeanReversionService : IDisposable
    {
        ...
        private static decimal scale { get; set; }
        public IConfiguration Configuration { get; set; }
        public AccountEnvironment AccountEnvironment { get; set; } = AccountEnvironment.Paper;
        ...
        public AlpacaMeanReversionService(string _symbol, decimal _scale, AccountEnvironment _enviro, IConfiguration configuration)
        {
            scale = _scale;
            symbol = _symbol;
            AccountEnvironment = _enviro;
            Configuration = configuration;
            API_KEY = configuration.GetSection("AlpacaConfig:API_KEY").Value;
            API_SECRET = configuration.GetSection("AlpacaConfig:API_SECRET").Value;
        }
        ...
        public async Task Run()
        {
            try
            {
                ...
                //Choose Environment
                if (AccountEnvironment == AccountEnvironment.Paper)
                {
                ...
                if (AccountEnvironment == AccountEnvironment.Live)
                {
                    alpacaTradingClient = Environments.Live.GetAlpacaTradingClient(new SecretKey(API_KEY, API_SECRET));
                    alpacaDataClient = Environments.Live.GetAlpacaDataClient(new SecretKey(API_KEY, API_SECRET));
                    // Connect to Alpaca's websocket and listen for updates on our orders.
                    alpacaStreamingClient = Environments.Live.GetAlpacaStreamingClient(new SecretKey(API_KEY, API_SECRET));
                }
            ...
            {
                Console.WriteLine("Rest Client Exception");
                return;
            }
        }
